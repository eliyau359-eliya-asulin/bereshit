...
# ---------------------------------------------------------------------
def test_admin_stock_update_writes_inventory_log_with_reason(client, db, cleanup):
    pid = TEST_PRODUCT_ID_START + 1
    make_product(db, pid, stock=20)
    cleanup.product_ids.append(pid)

    res = client.put(f"/api/products/{pid}", json={"stock": 12, "reason": "מכירה בחנות הפיזית"})
...
def test_admin_stock_update_without_reason_uses_default(client, db, cleanup):
    pid = TEST_PRODUCT_ID_START + 2
    make_product(db, pid, stock=5)
    cleanup.product_ids.append(pid)

    res = client.put(f"/api/products/{pid}", json={"stock": 9})
    assert res.status_code == 200, res.get_json()
...
def test_stock_update_that_does_not_change_stock_writes_no_log(client, db, cleanup):
    pid = TEST_PRODUCT_ID_START + 3
    make_product(db, pid, stock=7)
    cleanup.product_ids.append(pid)

    res = client.put(f"/api/products/{pid}", json={"name": "שם חדש"})
    assert res.status_code == 200, res.get_json()
...
# ---------------------------------------------------------------------
def test_admin_rejects_negative_stock(client, db, cleanup):
    pid = TEST_PRODUCT_ID_START + 4
    make_product(db, pid, stock=5)
    cleanup.product_ids.append(pid)

    res = client.put(f"/api/products/{pid}", json={"stock": -1})
    assert res.status_code == 400
    assert db.products.find_one({"_id": pid})["stock"] == 5


def test_admin_rejects_non_integer_stock(client, db, cleanup):
    pid = TEST_PRODUCT_ID_START + 5
    make_product(db, pid, stock=5)
    cleanup.product_ids.append(pid)

    res = client.put(f"/api/products/{pid}", json={"stock": 3.5})
    assert res.status_code == 400
    assert db.products.find_one({"_id": pid})["stock"] == 5
...
# ---------------------------------------------------------------------
def test_zeroing_stock_marks_product_out_and_restocking_reactivates(client, db, cleanup):
    pid = TEST_PRODUCT_ID_START + 6
    make_product(db, pid, stock=3, status="active")
    cleanup.product_ids.append(pid)

    res = client.put(f"/api/products/{pid}", json={"stock": 0, "reason": "תיקון מלאי"})
    assert res.status_code == 200, res.get_json()
    ...
    res2 = client.put(f"/api/products/{pid}", json={"stock": 10, "reason": "קבלת מלאי חדש"})
...
def test_explicit_status_in_patch_overrides_stock_auto_sync(client, db, cleanup):
    ...
    res = client.put(f"/api/products/{pid}", json={"stock": 15, "status": "draft"})
    assert res.status_code == 200, res.get_json()
...
# ---------------------------------------------------------------------
def test_full_online_and_physical_stock_lifecycle(client, db, cleanup):
    ...
    r2 = client.put(f"/api/products/{pid}", json={"stock": 12, "reason": "מכירה בחנות הפיזית"})
